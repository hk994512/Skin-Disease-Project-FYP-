...
mixin GoogleSignInMixin<T extends StatefulWidget> on State<T> {
  ...
  Future<void> signInWithGoogle() async {
    ...
    try {
      ...
      // ✅ Step 2: Get server auth which contains BOTH
      //    accessToken AND idToken correctly
      final GoogleSignInServerAuthorization? serverAuth = await googleUser
          .authorizationClient
          .authorizeServer(['email', 'profile', 'openid']);
      ...
      //    idToken  → comes from serverAuth.serverAuthCode (this IS the idToken)
      //    accessToken → comes from clientAuth.accessToken
      final credential = GoogleAuthProvider.credential(
        idToken: serverAuth.serverAuthCode, // ✅ correct
        accessToken: clientAuth.accessToken, // ✅ correct
      );
